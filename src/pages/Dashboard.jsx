...
export default function Dashboard() {
  ...
  const stats = useMemo(() => {
    ...
    const termine = cases.filter(c => c.status === 'Terminé').length;
    const slaDepassé = cases.filter(c => {
      if (!c.sla_due_at || c.status === 'Terminé') return false;
      return new Date(c.sla_due_at) < new Date();
    }).length;

    return { total, enAttente, enCours, termine, slaDepassé };
  }, [cases]);

  // Statistiques par médecin pour les cas terminés
  const doctorStats = useMemo(() => {
    const completedCases = cases.filter(c => c.status === 'Terminé' && c.assigned_derm_id && c.assigned_derm_name);
    const stats = {};
  ...
  const filteredDoctorCases = useMemo(() => {
    if (selectedDoctor === 'all') {
      return cases.filter(c => c.status === 'Terminé' && c.assigned_derm_id);
    }
    return cases.filter(c => c.status === 'Terminé' && c.assigned_derm_id === selectedDoctor);
  ...
  const getSLAStatus = (caseRecord) => {
    if (!caseRecord.sla_due_at || caseRecord.status === 'Terminé') return null;
    const now = new Date();
  ...
  if (isLoading || !isAuthenticated || casesLoading) {
    return (
          ...
          <Card className="p-6 bg-red-50">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-red-600">{stats.slaDepassé}</p>
              </div>
                  ...
                  return (
                    <tr key={caseRecord.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                        {new Date(caseRecord.created_date).toLocaleDateString('fr-FR')}
                      </td>
            ...
            <Card className="p-6 mb-6">
              <div className="flex items-center gap-4">
                <Select value={selectedDoctor} onValueChange={setSelectedDoctor}>
                  ...
                  <SelectContent>
                    <SelectItem value="all">Tous les médecins ({cases.filter(c => c.status === 'Terminé' && c.assigned_derm_id).length})</SelectItem>
                    {doctorStats.map(doc => (
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {filteredDoctorCases.map((caseRecord) => (
                      <tr key={caseRecord.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                          {caseRecord.closed_at ? new Date(caseRecord.closed_at).toLocaleDateString('fr-FR') : '-'}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
